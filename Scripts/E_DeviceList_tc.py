...
class DeviceListTest(unittest.TestCase):
    ...
    @BeautifulReport.add_img('002_store_search_fail')
    def test_002_Verify_input_shop_query(self):
        ...
        try:  # 传参 需要查询的门店名称
            ...
            self.c.save_img('002_store_search_fail')
            InsertLog().debug(msg)
            raise BaseException

    @BeautifulReport.add_img('003_device_Imei_fail')
    def test_003_Verify_DeviceIMEI(self):
        ...
        try:  # 传参 需要查询的设备IMEI
            ...
            self.c.save_img('003_device_Imei_fail')
            InsertLog().debug(msg)
            raise BaseException

    @BeautifulReport.add_img('004_device_status_fail')
    def test_004_Verify_Device_LockState(self):
        ...
        try:
            ...
            self.c.save_img('004_device_status_fail')
            InsertLog().debug(msg)
            raise BaseException
    ...
    @BeautifulReport.add_img('006_edit_fail')
    def test_006_Verify_Edit_Success(self):
        ...
        try:
            self.DeviceBusiness()
            self.assertTrue(self.c.verify_edit_success())
        except (BaseException, AssertionError) as msg:
            self.c.save_img('006_edit_fail')
            InsertLog().debug(msg)
            raise BaseException
